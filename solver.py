...
class Solver:
    ...
    def __init__(self, data_path):
        ...
        # Define alpha as a continuous variable
        self.alpha = model.addVar(vtype=gb.GRB.CONTINUOUS, name="alpha")
            ...
            if (time == 0) or (time == self.data.T):
                ...
                model.addConstr(
                    gb.quicksum(
                        self.X_as[index]
                        for index in range(len(self.data.arcs))
                        if self.data.arcs[index][3] == job
                    ) - gb.quicksum(
                        self.X_as[index]
                        for index in range(len(self.data.arcs))
                        if self.data.arcs[index][1] == job
                    ) == 0
                )
